document.addEventListener('DOMContentLoaded', () => {
    ...
    function startNightSky(sky) {
        ...
        for (let i = 0; i < 200; i++) { createAndAppend(sky, 'div', 'star', { width: `${Math.random()*2+1}px`, height: `${Math.random()*2+1}px`, left: `${Math.random()*100}%`, top: `${Math.random()*100}%`, animationDuration: `${3+Math.random()*2}s`, animationDelay: `${Math.random()*5}s`, '--start-opacity': Math.random()*0.7+0.3 }); }
        for (let i = 0; i < 100; i++) {
             const angle = (i / 100) * Math.PI * 4; const distance = (i / 100) * 30;
    ...
    function startMountains(container) {
        ...
        `;
        for (let i = 0; i < 8; i++) { createAndAppend(container, 'div', 'mountain-cloud', { width: `${Math.random()*40+20}vw`, height: `${Math.random()*10+5}vh`, left: `${Math.random()*100}vw`, top: `${Math.random()*40+10}%`, opacity: Math.random()*0.4+0.3, animationDuration: `${40+Math.random()*40}s` }); }
        addInterval(() => {
    ...
    function startBeach(container) {
        ...
        for (let i = 0; i < 4; i++) {
            const wave = createAndAppend(container, 'svg', 'wave', { height: `${15-i*3}%`, zIndex: 10-i, animationDuration: `${3+Math.random()*2}s`, animationDelay: `${i*0.5}s` });
            wave.setAttribute('viewBox', '0 0 1200 100');
            createAndAppend(wave, 'path', '', { fill: `rgba(59, 130, 246, ${0.3 - i * 0.05})`, animationDuration: `${3+Math.random()*2}s`, animationDelay: `${i*0.5}s` });
        }
        for (let i = 0; i < 6; i++) { createAndAppend(container, 'div', 'beach-cloud', { width: `${Math.random()*30+15}vw`, height: `${Math.random()*8+4}vh`, left: `${Math.random()*100}vw`, top: `${Math.random()*30+5}%`, opacity: Math.random()*0.4+0.6, animationDuration: `${60+Math.random()*40}s` }); }
        addInterval(() => {
